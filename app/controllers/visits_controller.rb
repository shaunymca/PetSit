class VisitsController < ApplicationController
  ...
  def index
    if current_user.has_role? :admin or current_user.has_role? :default or current_user.has_role? :basic or current_user.has_role? :standard or current_user.has_role? :plus or current_user.has_role? :advanced or current_user.has_role? :premier
      ...
      @print_visits = current_user.visits.where(['"visit_date" >= (:week_start) AND "visit_date" < (:weekend)', {week_start: Chronic.parse("Last week's Sunday midnight"), weekend: Chronic.parse("This week's Sunday")}]).order(:visit_date)
      @admin_visits = current_user.account.visits.where(['"visit_date" >= (:week_start) AND "visit_date" < (:weekend)', {week_start: Chronic.parse("Last week's Sunday midnight"), weekend: Chronic.parse("This week's Sunday")}]).order(:visit_date)
    else
