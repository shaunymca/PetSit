class VisitBlock < ActiveRecord::Base
  attr_accessible :id, :monday, :thursday, :user_name, :user_id, :tuesday, :american_end_date, :american_start_date, :visit_price, :visit_type, :wednesday, :friday, :saturday, :sunday, :time_text, :client_id, :client_name, :client_prices_id, :visits_attributes
  ...
  def user_name=(name)
    self.user = User.find_by_name(id) if name.present?
  end
    ...
    def new_visit  
      ...
      while day <= dates
        ...
        if (day_of_week == 'sunday' and sunday == true) or (day_of_week == 'monday' and monday == true) or (day_of_week == 'tuesday' and tuesday == true) or (day_of_week == 'wednesday' and wednesday == true) or (day_of_week == 'thursday' and thursday == true) or (day_of_week == 'friday' and friday == true) or (day_of_week == 'saturday' and saturday == true)
          visits.create(:user_id => user_id, :visit_price => visit_price, :visit_type => visit_type, :client_id => client_id, :visit_date => date, :end_time => date_end, :visit_block_id => id)
          day += 1
        else
          day += 1
        end
